...
class AppliedSlotsViewModel(private val mainRepository: MainRepository, private val token: String, private val context: Context? = null) :
    ViewModel() {
    ...
    private fun fetchWorkerSlots() {
        ...
        compositeDisposable.add(
            mainRepository.getAppliedSlots(token)
    ...
    fun acceptInvite(id: Int) {
        compositeDisposable.add(
            mainRepository.acceptInvite(id, token)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({ response ->
    ...
    fun declineInvite(id: Int) {
        compositeDisposable.add(
            mainRepository.declineInvite(id, token)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({ response ->
                    Toast.makeText(context, response, Toast.LENGTH_SHORT).show()
